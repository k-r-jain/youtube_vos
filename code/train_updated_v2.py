...
cost_fn = nn.BCELoss()
optimizer = torch.optim.Adam(list(initializer.parameters()) + list(encoder.parameters()) + list(convlstmcell_encoder.parameters()) + list(convlstmcell_decoder.parameters()) + list(h_convolution.parameters()) + list(decoder.parameters()), lr = lr)
...
for epoch in range(num_epochs):
	...
	for i, sample in enumerate(ytvos):
		...
		for i in range(yhat_list[0].shape[0]):
			...
			for j in range(len(yhat_list)):
				...
				gen_mask = np.array(PIL.Image.fromarray(gen_mask).resize((1280, 720), resample = PIL.Image.BILINEAR))
	...
	if (epoch + 1) % SAVE_EVERY_N_EPOCH == 0:
		...
		torch.save({
			...
			'h_convolution': h_convolution.state_dict(),
			'loss': epoch_avg_loss,
